...
async function findIp (host) {
  ...
  $('#dnsinfo>tr').each(function (i, element) {
...
function updateMd (content) {
  ...
  if (needUpdate) {
    ...
    const replacedMdContent = template.replace('{hostContent}', nextHostContent).replace("{update_time}", updateTime)
...
async function updateHosts () {
  ...
  try {
    updateMd(tpl.replace("{content}", generatedContent).trim());
  } catch (err) {
